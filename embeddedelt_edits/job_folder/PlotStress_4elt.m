...
basedir='C:/Users/Valerie/Documents/GitHub/flagshyp/embeddedelt_edits/job_folder/explicit_embedded_4el/';
set(0,'defaultfigurecolor',[1 1 1]);
file = [basedir 'explicit_embedded_4el-OUTPUT.txt'];
...
stress_elt3=zeros(numsteps, 6);
stress_elt4=zeros(numsteps, 6);
...
for j=1:numsteps
    ...
    for i = 1:7
        ...
        stress_elt3(j,:)=fscanf(fid, formspec, [1,6]);
    stress_elt3(j,:);
    for i = 1:7
        tline=fgetl(fid);
    end
    stress_elt4(j,:)=fscanf(fid, formspec, [1,6]);
    stress_elt4(j,:);
    for i = 1:9
        tline=fgetl(fid);
    end
...
hold on; grid on;
fig=gcf; fig.Position=graphsize;
plot([1:4:numsteps], stress_elt3(1:4:numsteps,1));
% xlim([1 740]);
% ylim([-1E7 1E7]);
xlabel("Step Number");
ylabel("Stress (Pa)");
